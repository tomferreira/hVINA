      ...
      return {
        ...
        link: function(scope) {
          ...
          function render(root) {
            ...
            function showDocs(nNode) {
              $('#docs_viewer #content').html("");
              $('#docs_viewer #pagination').remove();

              if (nNode.documents === undefined)
                return;

              $('#docs_viewer').append("<ul id='pagination' class='pagination-sm'></ul>");

              $('#docs_viewer #pagination').twbsPagination({
                totalPages: Math.max( 1, nNode.documents.length / 25 ),
                visiblePages: 5,
            ...
            function pagechange(event, page) {
              ...
              end = Math.min( start + docs_per_page, currentRoot.documents.length );

              var html = "<ol start=" + (start+1) + ">";

              for(i = start; i < end; ++i) { 
                html += "<li><a target=_blank href='" + currentRoot.links[i] + "'>" + currentRoot.documents[i] + "</a></li>";
              }

              html += "</ol>";

              $('#docs_viewer #content').html(html);
            }
